...
class _ResponsePageState extends State<ResponsePage> {
  ...
  void _fetchResponse() async {
    ...
    await Future.delayed(const Duration(seconds: 3)); // Remove this in real use case
    setState(() {
      responseText = widget.responseText; // Assume text is generated dynamically
      _isLoading = responseText.isEmpty; // Stop shimmer when response is available
    });
  }
      ...
      builder: (context, orientation, deviceType) {
        return Scaffold(
          ...
          body: SingleChildScrollView(
            child: 
            _isLoading ? _buildFullPageShimmer() : _buildResponse(),
          )
        );
      },
    );
